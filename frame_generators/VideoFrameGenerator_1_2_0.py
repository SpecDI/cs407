...
"""
This generator cuts off the action tube at the max frames_per_step and pads
the action tube with empty images.
"""
...
def list_action_tubes(dirpath, subdir, class_indices):
    classes = []
    action_tubes = []

    classes.append(class_indices[subdir])
    action_tubes.append(subdir + '/' + dirpath)
    return classes, action_tubes
...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
        ...
        self.frames_per_step = frames_per_step  #frames per action tube
        self.image_data_generator = image_data_generator
        ...
        # 1. [Change] Count the number of action tubes and classes
        self.samples = 0
        ...
        for subdir in classes:
            for action_tube in sorted(os.listdir((os.path.join(directory, subdir)))):
                sum +=1
        ...
        for subdir in classes:
            for dirpath in sorted(os.listdir((os.path.join(directory, subdir)))):
                results.append(list_action_tubes(dirpath, subdir, self.class_indices))
