...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
        ...
        # 1. [Change] Count the number of action tubes
        self.samples = len(os.listdir(directory))

        print('Got %d action tubes belonging to 12 classes.' % self.samples)

        # 2. [Change] Gets a list of absolute paths to action tubes
        results = []

        self.filenames = [directory + '/' + subdir for subdir in os.listdir(directory)]
    ...
    def next(self):
        ...
        for i in range(len(index_array)):
            ...
            ids[i] = action_tube_dir.split('/')[-1]
            # print(ids)
            loop_count = 0
            frame_count = 0
            # get the list of frames for this target.
            fname_list = sorted(os.listdir(action_tube_dir))
            x = []
